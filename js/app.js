...
//   };
  async function connectNami() {
        ...
        console.log(api);
        document.getElementById('wallet').innerHTML = 'Nami'; //not working
  ...
  async function connectEternl(){
      console.log(window.cardano.eternl);
      let api2 = await window.cardano.eternl.enable();
      console.log(api2);
  };

  
// async function connectEternl() {
//   let Eternl = undefined;
// const IEternl = Wallet.getInterface( Wallet.Names.Eternl );

// if( 
//     IEternl.isInjected() // equivalent to ```Wallet.has( Wallet.Names.Eternl )```
// )
// {
//     if(
//         IEternl.isAviable() // if enabled before returns true, no need to perform async calls
//     )
//     {
//         Eternl = Wallet.Eternl; // Wallet.get( Wallet.Names.Eternl ) will do the same
//     }
// }
// else // async code needed
// {
//     if(
//         await IEternl.isEnabled() // @returns {Promise<boolean>} as per CIP0030 definition
//     )
//     {
//         Eternl = Wallet.Eternl; // same as before
//     }
//     else
//     {
//         await Wallet.enable( Wallet.Names.Eternl ); // makes the object aviable
//         Eternl = Wallet.Eternl; // same as before
//     }
// }
// }
